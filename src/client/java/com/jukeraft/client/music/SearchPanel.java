...
final class SearchPanel {
    ...
    static void render(GuiGraphics graphics, float mouseX, float mouseY, boolean interactive,
        ...
        for (int i = 0; i < shown.size(); i++) {
            ...
            if (rowY + RESULT_H >= listY && rowY <= listY + listH) {
                ...
                ResourceLocation thumb = ThumbnailCache.get(r.thumbnailUrl());
                RoundedRectRenderer.fill(graphics, PAD + 3f, rowY + 3f, 32f, 32f, 6f, provider.thumbBg);
                if (thumb != null) {
                    graphics.blit(thumb, (int) (PAD + 3f), (int) (rowY + 3f), 0, 0, 32, 32, 32, 32);
                }
    ...
    private static void drawModePill(GuiGraphics graphics, Font font, Rect rect, String label, boolean selected,
