...
public final class AccountsPanel {
    ...
    public static void render(GuiGraphics graphics, float mouseX, float mouseY, boolean interactive,
    ...
    private static void renderInfoTab(GuiGraphics graphics, Font font, float mouseX, float mouseY, boolean interactive, float contentY) {
        float y = contentY;
    ...
    private static void drawLinkButton(GuiGraphics graphics, Font font, Rect rect, String label,
    ...
    private static void renderAccountsTab(GuiGraphics graphics, Font font, float contentY, float mouseX, float mouseY,
        ...
        if (accounts.isEmpty()) {
            ...
            for (YtAuthSession.AccountInfo account : accounts) {
                ...
                if (rowY + ROW_H >= listY && rowY <= listY + listH) {
                    ...
                    ResourceLocation photo = account.photoUrl() != null ? ThumbnailCache.get(account.photoUrl()) : null;
                    RoundedRectRenderer.fill(graphics, row.x + 4f, rowY + 4f, avatarSize, avatarSize, avatarSize / 2, provider.thumbBg);
                    if (photo != null) {
                        graphics.blit(photo, (int) (row.x + 4f), (int) (rowY + 4f), 0, 0, (int) avatarSize, (int) avatarSize, (int) avatarSize, (int) avatarSize);
                    } else {
    ...
    private static void drawPill(GuiGraphics graphics, Font font, Rect rect, String label, boolean selected,
