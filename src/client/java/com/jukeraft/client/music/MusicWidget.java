...
public final class MusicWidget {
    ...
    static final ResourceLocation FONT_REGULAR = ResourceLocation.fromNamespaceAndPath("minecraft", "default");
    static final ResourceLocation FONT_BOLD = FONT_REGULAR;
    ...
    public static void render(GuiGraphics graphics, int mouseX, int mouseY, boolean interactive) {
        if (!visible) {
            return;
        }
        refreshTheme();
        ...
        graphics.pose().translate(mainX, renderY, 0);
        ...
        if (fxAnimValue > 0.001f) {
            ...
            graphics.pose().pushPose();
            graphics.pose().translate(fxX, fxY, 0);
            graphics.pose().scale(scale, scale, 1f);
            graphics.pose().translate(anchorLocalX, anchorLocalY, 0);
        ...
        if (searchAnimValue > 0.001f) {
            ...
            graphics.pose().pushPose();
            graphics.pose().translate(searchX, searchY, 0);
            graphics.pose().scale(scale, scale, 1f);
            graphics.pose().translate(anchorLocalX, anchorLocalY, 0);
            graphics.pose().scale(popScale, popScale, 1f);
            graphics.pose().translate(-anchorLocalX, -anchorLocalY, 0);
    ...
    private static void renderCard(GuiGraphics graphics, PlaybackState state, float mouseX, float mouseY,
        ...
        ResourceLocation thumb = noTrack ? null : ThumbnailCache.get(state.thumbnailUrl());
        RoundedRectRenderer.fill(graphics, contentX, cursorY, THUMB, THUMB, THUMB_RADIUS, COL_THUMB_BG);
        if (thumb != null) {
            graphics.blit(thumb, (int) contentX, (int) cursorY, 0, 0, (int) THUMB, (int) THUMB, (int) THUMB, (int) THUMB);
        }
    ...
    private static float renderSeekRow(GuiGraphics graphics, Font font, PlaybackState state, float left, float right, float y) {
    ...
    private static float renderControlsRow(GuiGraphics graphics, PlaybackState state, float left, float right, float y,
    ...
    private static void renderQueuePanel(GuiGraphics graphics, Font font, PlaybackState state, float left, float right,
        ...
        for (PlaybackState.QueueItem item : queue) {
            if (rowY + QUEUE_ITEM_H >= y && rowY <= y + panelH) {
                ...
                ResourceLocation thumb = ThumbnailCache.get(item.thumbnailUrl());
                RoundedRectRenderer.fill(graphics, left + 3f, rowY + 3f, 26f, 26f, 5f, COL_THUMB_BG);
                if (thumb != null) {
                    graphics.blit(thumb, (int) (left + 3f), (int) (rowY + 3f), 0, 0, 26, 26, 26, 26);
                }
    ...
    static void drawText(GuiGraphics graphics, Font font, String text, float localX, float localY, int color) {
        drawText(graphics, font, text, localX, localY, color, FONT_REGULAR);
    }

    static void drawText(GuiGraphics graphics, Font font, String text, float localX, float localY, int color, ResourceLocation fontId) {
        Component styled = Component.literal(text).setStyle(Style.EMPTY.withFont(fontId));
        graphics.drawString(font, styled, (int) localX, (int) localY, color, false);
    }

    static int textWidth(Font font, String text, ResourceLocation fontId) {
        return font.width(Component.literal(text).setStyle(Style.EMPTY.withFont(fontId)));
    }

    static void drawTextBoosted(GuiGraphics graphics, Font font, String text, float localX, float localY, int color, ResourceLocation fontId, float boost) {
        Component styled = Component.literal(text).setStyle(Style.EMPTY.withFont(fontId));
        graphics.pose().pushPose();
        graphics.pose().translate(localX, localY, 0);
        graphics.pose().scale(boost, boost, 1f);
        graphics.drawString(font, styled, 0, 0, color, false);
        graphics.pose().popPose();
    }

    static void iconButton(GuiGraphics graphics, String id, float x, float y, float size,
    ...
    private static void iconButtonSmall(GuiGraphics graphics, String id, float x, float y,
                                         float mouseX, float mouseY, boolean interactive, boolean active) {
        iconButton(graphics, id, x, y, ICON_BTN_SMALL, mouseX, mouseY, interactive, active);
    }

    private static void playButton(GuiGraphics graphics, float x, float y, float mouseX, float mouseY,
    ...
    static String truncate(Font font, String text, int maxWidth, ResourceLocation fontId) {
        Style style = Style.EMPTY.withFont(fontId);
