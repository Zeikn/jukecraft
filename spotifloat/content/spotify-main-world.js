(() => {
  if (window.__spotifloatFXInstalled) return;
  window.__spotifloatFXInstalled = true;
  ...
  function setupGraphForElement(el) {
    try {
      ...
      window.postMessage({ source: "spotifloat-fx-status", ready: true }, "*");
      console.log("[SpotiFloat] FX graph attached to media element", el);
    } catch (err) {
      console.warn("[SpotiFloat] FX graph setup failed, falling back to passthrough:", err);
      try {
        if (sharedContext) {
  ...
  window.addEventListener("message", (event) => {
    ...
    if (!data || data.source !== "spotifloat-fx-command") return;
    handleFXCommand(data.payload);
  });

  window.__spotifloatFXDebug = {
