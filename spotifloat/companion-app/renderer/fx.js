...
const bandSliders = BANDS.map((band, index) => {
  ...
  slider.addEventListener("input", () => {
    window.spotifloat.sendCommand("eq-band", { index, gainDb: Number(slider.value) });
  });
...
Object.keys(EQ_PRESETS).forEach((name) => {
  ...
  btn.addEventListener("click", () => {
    ...
    window.spotifloat.sendCommand("eq-preset", { name });
  });
...
reverbEl.addEventListener("input", () => {
  ...
  window.spotifloat.sendCommand("reverb-wet", { value: Number(reverbEl.value) / 100 });
});

widthEl.addEventListener("input", () => {
  widthValueEl.textContent = `${widthEl.value}%`;
  window.spotifloat.sendCommand("stereo-width", { value: Number(widthEl.value) / 100 });
});
...
document.getElementById("fx-reset").addEventListener("click", () => {
  ...
  window.spotifloat.sendCommand("fx-reset");
});
...
window.spotifloat.onFXState((state) => {
  if (!state) return;
